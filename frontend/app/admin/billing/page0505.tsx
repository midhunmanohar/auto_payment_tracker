...
export default function BillingDashboard() {
    ...
    // NEW: Function to instantly generate and download a CSV report of the current screen
    const downloadCSV = () => {
        if (bills.length === 0) return;

        // 1. Create the Header Row
        const headers = ["Flat Number", "Maintenance Due", "Water Due", "Total Due", "Status", "Phone Number"];
        
        // 2. Loop through the currently viewed bills to create the data rows
        ...
        // 3. Combine headers and rows with commas and newlines
        const csvContent = [
            headers.join(","),
            ...rows.map(e => e.join(","))
        ].join("\n");

        // 4. Create a downloadable file in the browser
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto text-black relative">
            ...
            <div className="bg-white p-6 rounded-lg shadow border-l-4 border-indigo-500">
                {/* NEW: Added flex box to align the header and the new download button */}
